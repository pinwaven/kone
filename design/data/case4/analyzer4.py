...
def plot_combined_waveform(analyzer1, analyzer2, analyzer3, analyzer4, analyzer5, 
    ...
    try:
        ...
        # 绘制第一个分析器的数据（蓝色）
        ax1 = analyzer1.plot_waveform(color='#FF0000', label=f"{analyzer1.file_path.name}", ax=ax,mark_points=[370, 533])
        
        # 绘制第二个分析器的数据（红色）
        ax2 = analyzer2.plot_waveform(color='#008000', label=f"{analyzer2.file_path.name}", ax=ax, mark_points=[374, 537])
        
        # 绘制第三个分析器的数据（蓝色）
        ax3 = analyzer3.plot_waveform(color='#0000FF', label=f"{analyzer3.file_path.name}", ax=ax, mark_points=[370, 533])
        
        # 绘制第四个分析器的数据（红色）
        ax4 = analyzer4.plot_waveform(color='#FFA500', label=f"{analyzer4.file_path.name}", ax=ax, mark_points=[374, 537])
        
        # 绘制第一个分析器的数据（蓝色）
        ax5 = analyzer5.plot_waveform(color='#800080', label=f"{analyzer5.file_path.name}", ax=ax, mark_points=[370, 533])
        
        # 绘制第二个分析器的数据（红色）
        ax6 = analyzer6.plot_waveform(color='#00CCCC', label=f"{analyzer6.file_path.name}", ax=ax, mark_points=[374, 537])
        
        # 绘制第三个分析器的数据（蓝色）
        ax7 = analyzer7.plot_waveform(color='#FF00FF', label=f"{analyzer7.file_path.name}", ax=ax, mark_points=[370, 533])
        
        # 绘制第四个分析器的数据（红色）
        ax8 = analyzer8.plot_waveform(color='#BFBF00', label=f"{analyzer8.file_path.name}", ax=ax, mark_points=[374, 537])
        
        # 绘制第三个分析器的数据（蓝色）
        ax9 = analyzer9.plot_waveform(color='#FF80CC', label=f"{analyzer9.file_path.name}", ax=ax, mark_points=[370, 533])
        
        # 绘制第四个分析器的数据（红色）
        ax10 = analyzer10.plot_waveform(color='#006600', label=f"{analyzer10.file_path.name}", ax=ax, mark_points=[374, 537])
...
def main():
    try:
        analyzer1 = DataAnalyzer("001.bin")
        analyzer2 = DataAnalyzer("004.bin")
        analyzer3 = DataAnalyzer("15.bin")
        analyzer4 = DataAnalyzer("3534.bin")
        analyzer5 = DataAnalyzer("001.bin")
        analyzer6 = DataAnalyzer("004.bin")
        analyzer7 = DataAnalyzer("15.bin")
        analyzer8 = DataAnalyzer("3534.bin")
        analyzer9 = DataAnalyzer("15.bin")
        analyzer10 = DataAnalyzer("3534.bin")
